...
3. **Add the existing table** to the solution so the app's references travel
   with it (**Add existing → Table**, add *as-is* — do **not** recreate):
   - **`new_property`** only. This build is a single-table console; the related
     tables (`new_unitconfiguration`, `cr55d_propertycontact`,
     `new_managementagreementterm`, `new_propertytypeclassification`, `contact`)
     are out of scope — don't add them.
   - When prompted, add the columns + relationships (you want the metadata, not
     the data rows).
...
2. **Add the Dataverse data source** (Data → Add data → **Dataverse**):
   **`new_property`** only — this is a single-table console. Do not add the
   related/link tables.
